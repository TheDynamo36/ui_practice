...
class DrinkDetailsDark extends StatefulWidget {
  ...
  const DrinkDetailsDark({Key key,  this.id, this.directResponse}) : super(key: key);
...
SharedPreferenceHelper prefs;
    ...
    return MaterialApp(
          ...
          home: SafeArea(
                      child: Scaffold(
                        ...
                        body: response == null ? Center(child: CircularProgressIndicator(backgroundColor: blackText,))
                  ...
                  Padding(
                        ...
                        child: Text("${response['strAlcoholic'] ?? "Unidentified"}", style: TextStyle(color: blackText),),
                        ),
                  ...
                  Padding(
                    ...
                    child: Column(
                      children: <Widget>[
                        RowText(text: "${response['strCategory']}", imgName: "categoryblack"),
                        RowText(text: "${response['strTags'] ?? "No Comments"}", imgName: "commentcolor",),
                        RowText(text: "${response['strGlass'].toString().length < 4 ? "-" : response['strGlass']}", imgName: "glassred"),
                      ],
                    ),
                  ),
                  ...
                  Container(
                    ...
                    height: size.height > 680 ? 165 : (size.height*24.5/100),
 ...
 void updateUI(){
   setState(() {
     for(int i = 1 ; i <= 15 ; i++){
       if(this.response['strIngredient$i'] == null || this.response['strIngredient$i'].toString().length < 3)
       break;
 ...
 getSahredPreferences() async{
   ...
   prefs.checkFavourite(response['idDrink']) ? setState((){_favourite = true;}) : setState((){_favourite = false;});
 }
 ...
 toggleBookmark(){
  ...
  _image(){
    return SizedBox(
                        ...
                        child: ClipRRect(
                                ...
                                child: Image(image: NetworkImage("${response['strDrinkThumb']}") ?? AssetImage("assets/noimg.jpg"),
                               ),
                        ),
                      );
  ...
  openDetails(String ingredient){
    Navigator.push(context, MaterialPageRoute(builder: (context) => IngredientsDetails(name: ingredient)));
    }
}
