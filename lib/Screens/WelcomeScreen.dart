...
class _WelcomeScreenState extends State<WelcomeScreen> {
          ...
          children: <Widget>[
            Expanded(
                child: ClipRRect(
                    borderRadius: BorderRadius.circular(10.0),
                    child: Image.asset(
                      'assets/categories/${images['$category']}.jpg',
                      fit: BoxFit.cover,
                    ))),
            Padding(
              ...
              child: Center(
                  child: Text(category,
                      style: TextStyle(
                          fontWeight: FontWeight.w600, color: whiteText),
                      maxLines: 1)),
            ),
          ],
        ),
  ...
  // //List<PopUpMenuItem> choices = [
  //   PopUpMenuItem(title: "Favourite"),
  //   PopUpMenuItem(title: "Refresh"),
  // ];
  ...
  Widget _buildAppBar() {
    return AppBar(
      title: Text(
        "Madira App",
        style: TextStyle(fontWeight: FontWeight.w600, color: whiteText),
      ),
      actions: <Widget>[
        Row(
          children: <Widget>[
            IconButton(icon: Icon(Icons.search),onPressed:() => {Navigator.pushNamed(context, '/ingredientsPage')},),
            IconButton(icon: Icon(Icons.favorite),onPressed:() =>{Navigator.push(
  ...
  openList(String data, String prefix) {
    prefix != "i"
        ? Navigator.push(
            ...
            MaterialPageRoute(
                builder: (context) =>
                    MainScreen(category: data, prefix: prefix),
                fullscreenDialog: true))
        : Navigator.push(
            context,
            MaterialPageRoute(
                builder: (context) => IngredientsDetails(name: data),
                fullscreenDialog: true));
  }
...
// class PopUpMenuItem {
//   final String title;
//   const PopUpMenuItem({this.title});
// }
